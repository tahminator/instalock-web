...
def register():
    ...
    email: Union[str, None] = request_json.get(
        'email') if request_json is not None else None
    password: Union[str, None] = request_json.get(
        'password') if request_json is not None else None
    ...
    if user:
        if user.verified is True:
            return {'code': '409', 'message': 'User already exists'}, 409
        else:
...
def verifyemail():
    ...
    try:
        email: Union[str, None] = jwt.decode(token, g.SECRET_KEY, algorithms="HS256")[
            'verify_email']  # type: ignore
...
def login():
    ...
    password: Union[str, None] = request_json.get(
        'password') if request_json is not None else None
    remember_me: Union[bool, None] = bool(request_json.get(
        'rememberMe')) if request_json is not None else None
    ...
    login_user(user, duration=datetime.timedelta(
        hours=3), remember=remember_me)
